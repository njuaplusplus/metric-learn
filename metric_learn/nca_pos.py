...
class NCA_POS(MahalanobisMixin, TransformerMixin):
  """Neighborhood Components Analysis (NCA)
  ...
  def __init__(self, init='auto', n_components=None,
    ...
    self.an93_pos_label = an93_pos_label
    self.an93_optimize_neg_neg = an93_optimize_neg_neg
    print(f'Create NCA_POS with pos_label={an93_pos_label}')
    super(NCA_POS, self).__init__(preprocessor)
  ...
  def fit(self, X, y):
    ...
    if self.an93_optimize_neg_neg:
        ...
        neg_neg_maxprob_mask = 0.
    optimizer_params = {'method': 'L-BFGS-B',
                        'fun': self._loss_grad_lbfgs,
                        'args': (X, pos_pos_maxprob_mask, pos_neg_minporb_mask, neg_neg_maxprob_mask, -1.0),
                        'jac': True,
                        'x0': A.ravel(),
  ...
  def _loss_grad_lbfgs(self, A, X, pos_pos_maxprob_mask, pos_neg_minporb_mask, neg_neg_maxprob_mask, sign=1.0):
    ...
    # Compute loss
    masked_p_ij = p_ij * pos_pos_maxprob_mask - p_ij * pos_neg_minporb_mask + p_ij * neg_neg_maxprob_mask
